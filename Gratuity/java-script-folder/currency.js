...
function convertNumberToWords(amount) {
  ...
  // Clean and split the amount
  const cleanedAmount = amount.toString().replace(/[^0-9.]/g, ""); // keep digits and dot only
  const [cedisStr, pesewasStr] = cleanedAmount.split(".");
  const cedis = parseInt(cedisStr || "0");
  const pesewas = parseInt((pesewasStr || "0").substring(0, 2).padEnd(2, "0"));
